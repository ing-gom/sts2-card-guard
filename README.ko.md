...
- **왼쪽** — 적용 범위 선택: 맨 위 **★ 모든 캐릭터**, 또는 개별 캐릭터(기본 + **커스텀 캐릭터**).
  [★ 모든 캐릭터]에서 차단하면 어떤 캐릭터로 플레이하든 차단되고, 같은 항목이 각 캐릭터 목록에도
  잠긴 상태로 표시되어 어떤 범위가 그 카드를 지웠는지 항상 추적할 수 있습니다. (v0.8.0)
...
- **두 플레이어 모두 모드가 설치돼 있어야 하며, v0.7.0 이상이어야 합니다.** 한쪽에 없거나 호스트 설정을
  확인할 수 없으면, Card Guard는 desync를 감수하는 대신 **그 런에서 필터링을 끕니다** — 접속을 끊는 일은
  없고, 그 판만 아무 동작도 하지 않습니다. v0.7.0 이전에는 설정 교환이 접속 중인 클라이언트와의 경합에서
  져서, 2인 로비에서는 이 폴백이 항상 발동했습니다(= 협동에서 필터가 한 번도 켜지지 않음).
